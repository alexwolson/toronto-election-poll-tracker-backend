...
def test_endpoint_reliability_gate_is_configured_and_the_bridge_qualifies() -> None:
    result = evaluate_mayoral_endpoint_qualification(load_historical_mayoral_corpus(ROOT))
    ...
    assert result.comparator.all_elections.lead_times == (MAYORAL_ENDPOINT_EVALUATION_LEAD_TIMES)
    assert result.bridge.all_elections.manifest == (result.comparator.all_elections.manifest)
    assert result.bridge.regular_elections_only.manifest == (
        result.comparator.regular_elections_only.manifest
    )
    assert result.all_elections.endpoint_relative.primary_metric == (MAYORAL_MODEL_FAMILY_PRIMARY)
    assert result.all_elections.endpoint_relative.log_loss_guard == (MAYORAL_MODEL_FAMILY_LOG_GUARD)
