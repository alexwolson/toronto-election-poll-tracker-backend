...
def test_multi_term_councillor_record_spans_the_boundary_change() -> None:
    ...
    bio = build_candidate_biography("per_0fbda58a4d4d53d2882886ec2ab57599", _results())
...
def test_returning_former_councillor_record() -> None:
    ...
    bio = build_candidate_biography("per_d4bb25c4053f5c59840f117e87f63ab6", _results())
...
def test_identity_is_exact_key_and_does_not_conflate_by_surname() -> None:
    # 'clayton jones' is a different person (canonical candidate_id per_9c0b0e8585935507b9d686a0063791dd);
...
def test_single_term_incumbent_record() -> None:
    bio = build_candidate_biography("per_4569490f5d825a0ca8c7320b499d8090", _results())
...
def test_build_all_biographies_covers_every_distinct_candidate() -> None:
    ...
    assert "per_0fbda58a4d4d53d2882886ec2ab57599" in bios
    assert "per_d4bb25c4053f5c59840f117e87f63ab6" in bios
    assert len(bios) == len({r.candidate_id for r in results if r.candidate_id})
    # a built biography contains exactly that key's appearances
    assert bios["per_0fbda58a4d4d53d2882886ec2ab57599"].council_wins == 5
